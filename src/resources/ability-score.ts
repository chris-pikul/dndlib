...
import { isPlainObject, validateArray, validateString } from '../utils';
import {
...
export default class AbilityScore
    extends Resource
    implements IAbilityScore, IValidatable
{
    /**
    ...
    protected static readonly strictValidateProps = (props: any): void => {
        ...
        strictValidateOptionalArrayProp(
            ...
            'AbilityScore',
            'skills',
            ReferenceSkill.strictValidateProps,
        );
    };
    ...
    constructor(props?: any) {
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof AbilityScore) {
                ...
                if (props.skills) {
                    this.skills = props.skills.map(
                        (ent: ReferenceSkill) => new ReferenceSkill(ent),
                    );
                }
                ...
                console.warn(
                    `Attempting to instantiate an AbilityScore object with an invalid parameter. Expected either an AbilityScore object, JSON object of properties, or a string. Instead encountered a "${typeof props}".`,
                );
            }
        }
    }
                ...
                if (prop) {
                    if (prop instanceof ReferenceSkill) {
                        return prop
                            ...
                            .map(
                                (err: string) =>
                                    `AbilityScore.skills[${ind}]: ${err}`,
                            );
                    }

                    return [
                        `AbilityScore.skills[${ind}] should be a ReferenceSkill object.`,
                    ];
                }
