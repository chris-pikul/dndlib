import { IReference, Lifestyle, makeOptionsArray, OptionsArray, ReferenceItem, ReferenceLanguage, ReferenceSkill, ResourceType, strictValidateOptionsArray } from '..';
import { IValidatable, JSONObject, PromiseValidation, ValidationErrors } from '../interfaces';
import Reference from '../reference';
import Resource from '../resource';
import RollTable from '../roll-table';
import TextSection from '../text-section';
import { isPlainObject } from '../utils';
import { strictValidateOptionalObjectProp, strictValidatePropsParameter, strictValidateRequiredArrayProp, strictValidateRequiredProp } from '../utils/errors';
...
export interface IBackgroundStartingLanguages extends IBackgroundStartingOptions<ReferenceLanguage> {
  wildcard ?: {
    amount : number;
    standard ?: boolean;
    exotic ?: boolean;
...
export default class Background extends Resource implements IBackground, IValidatable {
  ...
  constructor(props?:any) {
    ...
    if(props) {
      if(isPlainObject(props) || props instanceof Background) {
        ...
        console.warn(`Attempting to instantiate an Background object with an invalid parameter. Expected either a Background object, or JSON object of properties. Instead encountered a "${typeof props}".`);
      }
    }
  }

  public static StartingLanguages = class implements IBackgroundStartingLanguages, IValidatable {
    static readonly strictValidateProps = (props:any):void => {
      strictValidatePropsParameter(props, 'Background.StartingLanguages');

      strictValidateRequiredArrayProp(props, 'Background.StartingLanguages', 'starting', ReferenceLanguage.strictValidateProps);
      strictValidateOptionsArray(props.options, 'Background.StartingLanguages.options');
      strictValidateOptionalObjectProp(props, 'Background.StartingLanguages', 'wildcard', (prop:JSONObject):void => {
    ...
    constructor(props?:any) {
      if(props) {
        if(isPlainObject(props) || props instanceof Background.StartingLanguages) {
          ...
          console.warn(`Attempting to instantiate an Background.StartingLanguages object with an invalid parameter. Expected either a Background.StartingLanguages object, or JSON object of properties. Instead encountered a "${typeof props}".`);
        }
      }
    }
