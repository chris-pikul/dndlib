...
export default class Action extends Resource implements IAction, IValidatable {
  public static readonly strictValidateProps = (props:any):void => {
    ...
    strictValidateOptionalObjectProp(props, 'Action', 'variantSource', Source.strictValidateProps);
  ...
  constructor(props?:any) {
    ...
    if(props) {
      if(isPlainObject(props) || props instanceof Action) {
        ...
        console.warn(`Attempting to instantiate an Action object with an invalid parameter. Expected either an Action object, or JSON object of properties. Instead encountered a "${typeof props}".`);
      }
    }
  }
  ...
  validateSync = (parentErrs?:ValidationErrors):ValidationErrors => {
    ...
    if(this.isVariant) {
      if(this.variantSource) {
        validateObject(errs, 'Action', 'variantSource', this.variantSource, (prop:any):ValidationErrors => {
          if(prop instanceof Source) {
            const subErrs = prop.validateSync();
            return subErrs.map((err:string) => `Action.variantSource: ${err}`);
          }

          return [ `Action.variantSource should be a Source object, instead found "${typeof prop}"."` ];
        });
      } else {
        errs.push(`Action.variantSource is required if this action is marked as "isVariant".`);
      }
    }

    validateObject(errs, 'Action', 'timing', this.timing, (prop:any):ValidationErrors => {
      if(isPlainObject(prop)) {
        const subErrs:ValidationErrors = [];
        validateEnum(subErrs, 'Action::Timing', 'type', prop.type, TimingType);
        validateInteger(subErrs, 'Action::Timing', 'count', prop.count, {
          positive: true,
          minValue: 1,
        });
        return subErrs;
      ...
      return [ `Action.timing should be an object, instead found "${typeof prop}".` ];
    });
