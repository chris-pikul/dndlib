...
export default abstract class Resource implements IResource, IValidatable {
    ...
    constructor(props?: any, classProps?: IResourceClassProps) {
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof Resource) {
                ...
                console.warn(
                    `Attempting to instantiate a Resource object with an invalid parameter. Expected either a Resource object, or a plain JSON Object of properties. Instead encountered a "${typeof props}"`,
                );
            }
        }
        ...
        if (classProps && typeof classProps === 'object') {
            ...
            if (
                classProps.type &&
                typeof classProps.type === 'string' &&
                resourceTypeHas(classProps.type)
            )
    ...
    validate = (): PromiseValidation =>
        new Promise<ValidationErrors>((resolve) => {
            ...
            validateString(errs, clsName, 'id', this.id, {
                regexp: RegexpKabob,
            });
            validateString(errs, clsName, 'uri', this.uri, {
                regexp: RegexpURI,
            });
                ...
                (prop: any, ind: number): ValidationErrors => {
                    ...
                    if (prop) {
                        if (typeof prop !== 'string') {
                            subErrs.push(
                                `${clsName}.tags[${ind}] is not a string type, instead found "${typeof prop}".`,
                            );
                        } else if (testKabob(prop) === false) {
                            subErrs.push(
                                `${clsName}.tags[${ind}] "${prop}" does not match the kabob format.`,
                            );
                        }
                    } else {
                        subErrs.push(
                            `${clsName}.tags[${ind}] is a required string.`,
                        );
                    }
            ...
            this.description.validate().then((descErrs: ValidationErrors) => {
                ...
                errs.push(
                    ...descErrs.map(
                        (err: string) => `${clsName}.description: ${err}`,
                    ),
                );
