...
import { isPlainObject, validateObject, validateString } from './utils';
import {
    ...
    constructor(props?: any) {
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof TextSection) {
                ...
                console.warn(
                    `Attempting to instantiate a TextSection object with an invalid parameter. Expected either a TextSection object, or a plain JSON Object of properties. Instead encountered a "${typeof props}"`,
                );
            }
        }
    }
    ...
    validateSync = (): ValidationErrors => {
        ...
        validateObject(
            ...
            (ent: TextBlock) =>
                ent
                    .validateSync()
                    .map((err: string) => `TextSection.body -> ${err}`),
        );
