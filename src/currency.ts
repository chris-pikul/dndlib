 ...
 */
import { isPlainObject, positive, randomFloat } from './utils';
...
/**
 * A tuple-array holding all of the denominations. Breaks down as:
 *
 * ```js
 * const [
 *    copper,
...
export default class Currency implements ICurrency {
     ...
     */
    static reduce = (value: number, div = 10): Array<number> => [
        value % div,
    ...
    constructor(props: any = null, usePlatinum = true, useElectrum = false) {
        ...
        if (typeof props !== 'undefined' && props !== null) {
            if (props instanceof Currency || isPlainObject(props)) {
                ...
                console.warn(
                    `Attemnpting to instantiate AlignmentAxes with invalid property.`,
                );
            }
        }
    }
    ...
    /**
     * Returns this currency object as a tuple of denominations. Breaks down as:
     *
     * ```js
     * const [
     *    copper,
     ...
     */
    toString = (): string => {
        const denoms: Array<string> = [];

        if (this.copper) denoms.push(`${this.copper}CP`);
        if (this.silver) denoms.push(`${this.silver}SP`);
        if (this.electrum) denoms.push(`${this.electrum}EP`);
        if (this.gold) denoms.push(`${this.gold}GP`);
        if (this.platinum) denoms.push(`${this.platinum}PP`);

        if (denoms.length === 0) return 'Zero';
