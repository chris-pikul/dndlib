...
export default class TextBlock implements ITextBlock, IValidatable {
    ...
    constructor(props?: any) {
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof TextBlock) {
                ...
                this.plainText = props.plainText.filter(
                    (ent: any) => ent && typeof ent === 'string',
                );

                if (props.markdown) {
                    this.markdown = props.markdown.filter(
                        (ent: any) => ent && typeof ent === 'string',
                    );
                }

                if (props.html) {
                    this.html = props.html.filter(
                        (ent: any) => ent && typeof ent === 'string',
                    );
                }
            } else {
                console.warn(
                    `Attempting to instantiate a TextBody object with an invalid parameter. Expected either a TextBody object, or a plain JSON Object of properties. Instead encountered a "${typeof props}"`,
                );
            }
        }
    }
    ...
    validateSync = (): ValidationErrors => {
        ...
        if (this.plainText.length === 0) {
            errs.push(
                `TextBlock.plainText requires at least one entry, none found.`,
            );
        } else {
            validateArray(
                errs,
                ...
                (ent: any, ind: number) => {
                    if (!ent || typeof ent !== 'string') {
                        return [
                            `TextBlock.plainText[${ind}] is a "${typeof ent}", expected a string.`,
                        ];
                    }
                    return [];
                },
        ...
        validateArray(
            ...
            (ent: any, ind: number) => {
                if (!ent || typeof ent !== 'string') {
                    return [
                        `TextBlock.markdown[${ind}] is a "${typeof ent}", expected a string.`,
                    ];
                }
                return [];
            },
        ...
        validateArray(
            ...
            (ent: any, ind: number) => {
                if (!ent || typeof ent !== 'string') {
                    return [
                        `TextBlock.html[${ind}] is a "${typeof ent}", expected a string.`,
                    ];
                }
                return [];
            },
