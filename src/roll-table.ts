...
export class RollTableEntry implements IRollTableEntry, IValidatable {
    ...
    constructor(props?: any) {
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof RollTableEntry) {
                ...
                if (props.minimumValue) this.minimumValue = props.minimumValue;

                if (props.maximumValue) this.maximumValue = props.maximumValue;

                if (props.title) this.title = props.title;
            } else {
                // Console a warning
                console.warn(
                    `Attempting to instantiate a RollTableEntry object with an invalid parameter. Expected either a RollTableEntry object, or a plain JSON Object of properties. Instead encountered a "${typeof props}"`,
                );
            }
        }
    }
    ...
    validateSync = (): ValidationErrors => {
        ...
        if (this.value && this.value >= 0) {
            if (this.minimumValue) {
                errs.push(
                    `RollTableEntry requires either value or minimum/maximum. Cannot use both, found minimumValue to be set.`,
                );
            }
            if (this.maximumValue) {
                errs.push(
                    `RollTableEntry requires either value or minimum/maximum. Cannot use both, found maximumValue to be set.`,
                );
            }
        } else {
            if (!this.minimumValue) {
                errs.push(
                    `RollTableEntry requires a minimumValue to be set if not using value.`,
                );
            }
            if (!this.maximumValue) {
                errs.push(
                    `RollTableEntry requires a maximumValue to be set if not using value.`,
                );
            }
        }
    ...
    check = (val: number): boolean => {
        if (this.value) return val === this.value;
...
export default class RollTable implements IRollTable, IValidatable {
    ...
    public static strictValidateProps = (props: any): void => {
        ...
        strictValidateRequiredArrayProp(
            ...
            'RollTable',
            'results',
            RollTableEntry.strictValidateProps,
        );
    };
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof RollTableEntry) {
                ...
                this.results = props.results.map(
                    (ent: any) => new RollTableEntry(ent),
                );
            } else {
                console.warn(
                    `Attempting to instantiate a RollTable object with an invalid parameter. Expected either a RollTable object, or a plain JSON Object of properties. Instead encountered a "${typeof props}"`,
                );
            }
        }
    }
    ...
    validate = (): PromiseValidation =>
        new Promise<ValidationErrors>((resolve) => {
            ...
            if (this.results.length === 0) {
                ...
                const proms = this.results.map((ent: RollTableEntry) =>
                    ent.validate(),
                );
                ...
                Promise.all(proms).then((promErrs: Array<ValidationErrors>) => {
                    ...
                    promErrs.forEach(
                        (resErr: ValidationErrors, ind: number) => {
                            /*
                             * Iterate the individual results and conform them to better
                             * error messages
                             */
                            const indErrs: ValidationErrors = resErr.map(
                                (err: string) =>
                                    `RollTable.results[${ind}] ${err}`,
                            );
                            errs.push(...indErrs);
                        },
                    );
    ...
    validateSync = (): ValidationErrors => {
        ...
        if (this.results.length === 0) {
            ...
            validateArrayOfObjects(
                ...
                'results',
                this.results,
                (ent: RollTableEntry) => ent.validateSync(),
            );
        }
    ...
    get = (val: number): RollTableEntry | undefined =>
        this.results.find((ent: RollTableEntry) => ent.check(val));
}
