...
export type EAlignmentEntropy =
    (typeof AlignmentEntropies)[keyof typeof AlignmentEntropies];

export const alignmentEntropyHas = (key: string): boolean =>
    enumHas(AlignmentEntropies, key);
...
export type EAlignmentMorality =
    (typeof AlignmentMoralities)[keyof typeof AlignmentMoralities];

export const alignmentMoralityHas = (key: string): boolean =>
    enumHas(AlignmentMoralities, key);
    ...
    constructor(props?: any) {
        if (typeof props !== 'undefined' && props !== null) {
            if (props instanceof AlignmentAxes) {
                ...
                if (
                    props.entropy &&
                    typeof props.entropy === 'string' &&
                    alignmentEntropyHas(props.entropy)
                )
                    this.entropy = props.entropy;

                if (
                    props.morality &&
                    typeof props.morality === 'string' &&
                    alignmentMoralityHas(props.morality)
                )
                    this.morality = props.morality;
            } else {
                console.warn(
        ...
        switch (value) {
            ...
            default:
                return new TypeError(
                    `The alignment enumerated value "${value}" does not map to a valid Alignments.`,
                );
        }
    };
