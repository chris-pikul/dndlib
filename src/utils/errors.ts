import type { ArrayIteratorCB, JSONObject } from '../interfaces';
import { isPlainObject } from './json-object';

export type StrictArrayPropValidationCB = string | ArrayIteratorCB;

export const makeStrictValidateMissingProps = (className: string): Error =>
    new TypeError(
        `${className}.StrictValidateProps requires a valid parameter to check, none was given.`,
    );

export const makeStrictValidateMissingProp = (
    className: string,
    propName: string,
): Error => new TypeError(`Missing "${propName}" property for ${className}.`);

export const makeStrictValidateWrongType = (
    className: string,
    propName: string,
    expected: string,
    real: string,
    ...
    new TypeError(
        `${className} "${propName}" property must be a(n) ${expected}. Instead found "${real}".`,
    );

export const makeStrictValidateArrayWrongType = (
    className: string,
    propName: string,
    index: number,
    expected: string,
    ...
    new TypeError(
        `${className} "${propName}" array property must have index ${index} be a(n) ${expected}. Instead found "${real}".`,
    );
 ...
 */
export const strictValidatePropsParameter = (
    props: any,
    className: string,
): void => {
    if (!isPlainObject(props)) throw makeStrictValidateMissingProps(className);
};
...
export const strictValidateRequiredProp = (
    ...
    if (typeof props[propName] !== expectedType) {
        throw makeStrictValidateWrongType(
            className,
            propName,
            expectedType,
    ...
    if (typeof arrValidation === 'string') {
        ...
        (props[propName] as Array<any>).forEach((ent: any, ind: number) => {
            if (typeof ent !== arrValidation) {
                throw makeStrictValidateArrayWrongType(
                    className,
                    propName,
                    ind,
                    arrValidation,
