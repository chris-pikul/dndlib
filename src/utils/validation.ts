...
import { StringEnum, enumHas, enumValuesToString } from './enums';
...
export function testURI(input: string, allowEmpty = false): boolean {
    if (allowEmpty && input.length === 0) return true;
...
export const testIfInteger = (input: any): boolean =>
    input !== null && typeof input === 'number' && Number.isInteger(input);
 ...
 */
export const testIfPositiveInteger = (input: any): boolean =>
    testIfInteger(input) && input >= 0;
...
export const validateBoolean = (
    ...
    if (!optional && !prop) {
        inPlaceConcat(errs, [
            `${className}.${propName} is a required boolean.`,
        ]);
    }

    if (prop && typeof prop !== 'boolean') {
        inPlaceConcat(errs, [
            `${className}.${propName} is not a boolean type, instead found "${typeof prop}".`,
        ]);
    }
};
    ...
    if (prop) {
        if (typeof prop === 'number') {
            if (testIfInteger(prop) === false) {
                newErrs.push(
                    `${className}.${propName} "${prop}" is not an whole integer, remove any floating point decimals.`,
                );
            }

            if (options.positive && testIfPositiveInteger(prop) === false) {
                newErrs.push(
                    `${className}.${propName} "${prop}" should be a positive value.`,
                );
            }

            if (options.minValue && prop < options.minValue) {
                newErrs.push(
                    `${className}.${propName} "${prop}" is under the minimum allowed value ${options.minValue}.`,
                );
            }

            if (options.maxValue && prop > options.maxValue) {
                newErrs.push(
                    `${className}.${propName} "${prop}" is over the maximum allowed value ${options.minValue}.`,
                );
            }

            if (
                options.multiple &&
                testIfMultipleOf(prop, options.multiple) === false
            ) {
                newErrs.push(
                    `${className}.${propName} "${prop}" is not a multiplier of "${options.multiple}".`,
                );
            }
        } else {
            newErrs.push(
                `${className}.${propName} should be a number type, instead found "${typeof prop}".`,
            );
        }
    }
...
export const validateString = (
    ...
    if (prop) {
        if (typeof prop !== 'string') {
            newErrs.push(
                `${className}.${propName} should be a string type, instead found "${typeof prop}".`,
            );
        } else if (options.func && options.func(prop) === false) {
            newErrs.push(
                `${className}.${propName} failed the functional string test.`,
            );
        } else if (options.regexp && options.regexp.test(prop) === false) {
            newErrs.push(
                `${className}.${propName} failed the regular expression test "${options.regexp.toString()}".`,
            );
        } else if (options.absLength) {
            if (prop.length !== options.absLength) {
                newErrs.push(
                    `${className}.${propName} "${prop}" should have an exact length of ${options.absLength}, instead it has a length of ${prop.length}.`,
                );
            }
        } else if (options.minLength || options.maxLength) {
            if (options.minLength && prop.length < options.minLength) {
                newErrs.push(
                    `${className}.${propName} "${prop}" should have a minumum length of ${options.minLength}, instead it has a length of ${prop.length}.`,
                );
            }

            if (options.maxLength && prop.length > options.maxLength) {
                newErrs.push(
                    `${className}.${propName} "${prop}" should have a maximum length of ${options.maxLength}, instead it has a length of ${prop.length}.`,
                );
            }
        }
    }
...
export const validateEnum = (
    ...
    if (
        ...
        newErrs.push(
            `${className}.${propName} is a required enumerated string.`,
        );
    }
    ...
    if (prop) {
        if (typeof prop !== 'string') {
            newErrs.push(
                `${className}.${propName} should be an enumerated string type, instead found "${typeof prop}".`,
            );
        } else if (enumHas(enumClass, prop) === false) {
            newErrs.push(
                `${className}.${propName} "${prop}" is not a valid enumerated string, accepted values are ${enumValuesToString(
                    enumClass,
                )}.`,
            );
        }
    ...
    if (prop) {
        if (typeof cb === 'function') {
            if (Array.isArray(prop)) {
                const subErrs = prop
                    .map(cb)
                    .filter((ent: ValidationErrors) => ent.length > 0);
                inPlaceConcat<string>(newErrs, ...subErrs);
            } else {
                newErrs.push(
                    `${className}.${propName} should be an array type, instead found "${typeof prop}".`,
                );
            }
        } else {
            newErrs.push(
                `${className}.${propName} attempted to validate as an array with an invalid callback function.`,
            );
        }
    }
    ...
    if (prop) {
        if (typeof cb === 'function') {
            if (isPlainObject(prop)) {
                ...
                newErrs.push(
                    `${className}.${propName} should be an object type, instead found "${typeof prop}".`,
                );
            }
        } else {
            newErrs.push(
                `${className}.${propName} attempted to validate as an object with an invalid callback function.`,
            );
        }
    }
...
export const validateArrayOfObjects = (
    ...
    if (prop) {
        if (typeof cb === 'function') {
            ...
            newErrs.push(
                `${className}.${propName} attempted to validate as an array of objects with an invalid callback function.`,
            );
        }
    }
