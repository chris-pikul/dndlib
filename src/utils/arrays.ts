 ...
 */
export function inPlaceConcat<Type>(
    target: Array<Type>,
    ...argArr: Array<Array<Type>>
): number {
    return Math.max.apply(
        null,
        argArr.map((arr: Array<Type>) =>
...
export function strictValidateCountedArrayElem(
    props: any,
    elemType?: string,
): void {
    if (!props || typeof props !== 'object') {
        throw new TypeError(
            `StrictValidateCountedArrayElem requires a valid object parameter.`,
        );
    }
    ...
    if (elemType && typeof props.type !== elemType) {
        throw new TypeError(
            `CountedArrayElem "type" property must be a "${elemType}", instead found "${typeof props.type}".`,
        );
    }
    ...
    if (typeof props.count !== 'number') {
        throw new TypeError(
            `CountedArrayElem "count" property must be a number, instead found "${typeof props.count}".`,
        );
    }
}

export function strictValidateCountedArray(
    props: Array<any>,
    elemType?: string,
): void {
    if (!props || typeof props !== 'object' || Array.isArray(props) === false) {
        throw new TypeError(
            `StrictValidateCountedArray requires a valid array object parameter.`,
        );
    }

    props.forEach((ent: unknown) =>
        strictValidateCountedArrayElem(ent, elemType),
    );
}
