...
export const makeStrictValidateMissingProps = (className) => new TypeError(`${className}.StrictValidateProps requires a valid parameter to check, none was given.`);
export const makeStrictValidateMissingProp = (className, propName) => new TypeError(`Missing "${propName}" property for ${className}.`);
export const makeStrictValidateWrongType = (className, propName, expected, real) => new TypeError(`${className} "${propName}" property must be a(n) ${expected}. Instead found "${real}".`);
export const makeStrictValidateArrayWrongType = (className, propName, index, expected, real) => new TypeError(`${className} "${propName}" array property must have index ${index} be a(n) ${expected}. Instead found "${real}".`);
/**
 ...
 */
export const strictValidatePropsParameter = (props, className) => {
 ...
 */
export const strictValidateRequiredProp = (props, className, propName, expectedType) => {
    if (!props[propName])
 ...
 */
export const strictValidateRequiredObjectProp = (props, className, propName, objValidation) => {
    if (!props[propName])
        throw makeStrictValidateMissingProp(className, propName);
    if (!isPlainObject(props[propName]))
        throw makeStrictValidateWrongType(className, propName, 'object', typeof props[propName]);
    objValidation(props[propName]);
};
/**
 ...
 * @param expectedType The type expected
 */
export const strictValidateOptionalProp = (props, className, propName, expectedType) => {
    if (props[propName]) {
        if (typeof props[propName] !== expectedType)
            throw makeStrictValidateWrongType(className, propName, expectedType, typeof props[propName]);
    }
 ...
 * @param objValidation Callback to validate the property itself
 */
export const strictValidateOptionalObjectProp = (props, className, propName, objValidation) => {
    if (props[propName]) {
        if (!isPlainObject(props[propName]))
            throw makeStrictValidateWrongType(className, propName, 'object', typeof props[propName]);
        objValidation(props[propName]);
 ...
 * @param arrValidation Either a string, or function to validate entries against
 */
export const strictValidateOptionalArrayProp = (props, className, propName, arrValidation) => {
    if (props[propName]) {
        if (!Array.isArray(props[propName]))
            throw makeStrictValidateWrongType(className, propName, 'array', typeof props[propName]);
        if (typeof arrValidation === 'string') {
