...
import { enumHas, enumValuesToString, } from './enums';
...
export const testIfMultipleOf = (input, multiplier) => ((input % multiplier) === 0);
/**
 ...
 */
export const validateBoolean = (errs, className, propName, prop, optional = false) => {
    if (!optional && !prop)
 ...
 * @param optional Is this prop optional (null or undefined)?
 */
export const validateInteger = (errs, className, propName, prop, options = {}, optional = false) => {
    const newErrs = [];
    if (!optional && !prop)
 ...
 * @param optional Is this prop optional (null or undefined)?
 */
export const validateString = (errs, className, propName, prop, options = {}, optional = false) => {
    const newErrs = [];
 ...
 * @param optional Is this prop optional (null or undefined)?
 */
export const validateEnum = (errs, className, propName, prop, enumClass, optional = false) => {
    const newErrs = [];
    if (!optional && (!prop || (typeof prop === 'string' && prop.length === 0)))
 ...
 */
export const validateArray = (errs, className, propName, prop, cb, optional = false) => {
    const newErrs = [];
    if (!optional && !prop)
        newErrs.push(`${className}.${propName} is a required array.`);
    if (prop) {
        if (Array.isArray(prop)) {
            const subErrs = prop.map(cb).filter((ent) => ent.length > 0);
            inPlaceConcat(newErrs, ...subErrs);
        }
        else {
 ...
 * @returns Array of string errors
 */
export const validateObject = (errs, className, propName, prop, cb, optional = false) => {
    const newErrs = [];
    if (!optional && !prop)
 ...
 * @returns Array of string errors
 */
export const validateArrayOfObjects = (errs, className, propName, prop, cb, optional = false) => {
    const newErrs = [];
    if (!optional && !prop)
        newErrs.push(`${className}.${propName} is a required array.`);
    if (prop) {
        if (Array.isArray(prop)) {
            prop.forEach((ent, ind) => {
                const entErrs = cb(ent);
                if (entErrs.length > 0)
                    newErrs.push(...entErrs.map((err) => `${className}.${propName}[${ind}]: ${err}`));
            });
        }
        else {
            newErrs.push(`${className}.${propName} should be an array type, instead found "${typeof prop}".`);
