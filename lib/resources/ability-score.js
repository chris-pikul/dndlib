...
import { strictValidateOptionalArrayProp, strictValidatePropsParameter, strictValidateRequiredProp, } from '../utils/errors';
;
/**
 ...
 */
export default class AbilityScore extends Resource {
    /**
    ...
    constructor(props) {
        ...
        this.validate = () => {
            ...
            validateArray(errs, 'AbilityScore', 'skills', this.skills, (prop, ind) => {
                if (prop) {
                    if (prop instanceof ReferenceSkill)
                        return prop.validate().map((err) => `AbilityScore.skills[${ind}]: ${err}`);
        ...
        if (props) {
            if (isPlainObject(props) || props instanceof AbilityScore) {
                ...
                if (props.skills)
                    this.skills = props.skills.map((ent) => new ReferenceSkill(ent));
            }
